...
public class TicTacToe implements GenericGridGame<Character> {
	...
	private void doMove(TicTacToeMove move) {
		...
		fireGridChanged(move.x, move.y, 1, 1);
		// Change player
	...
	private Collection<GridListener<Character>> gridListeners = new ArrayList<>();

	private void fireGridChanged(int col, int row, int width, int height) {
		for (GridListener<Character> gridListener : gridListeners) {
			gridListener.gridChanged(this, col, row, width, height);
		}
	}
